...
var Line = React.createClass({
    propTypes: {
        lineColor: ReactPropTypes.string,
        lineWidth: ReactPropTypes.number.isRequired,
        fromRatio: ReactPropTypes.number.isRequired,
        toRatio: ReactPropTypes.number.isRequired,
        pointMode: ReactPropTypes.oneOf(["from", "to", "both", "none"])
            .isRequired,
        pointRadius: ReactPropTypes.number,
        pointBorderWidth: ReactPropTypes.number,
        pointBorderColor: ReactPropTypes.string,
        pointColor: ReactPropTypes.string
    },
    ...
    render: function() {
        ...
        var linePadding =
            this.props.pointMode != "none" ? this.props.pointRadius : 0;
        ...
        var lineStyle = {
            ...
            transform: [{ rotate: Math.round(rotate) + "deg" }, { scaleX }]
        };
        // points
        var pointStyle = {
        ...
        // from point
        var drawFromPoint =
            this.props.pointMode == "from" || this.props.pointMode == "both";
        if (drawFromPoint) {
            var fromPointStyle = {
                ...pointStyle,
            ...
            var fromPoint = <View key="from" style={fromPointStyle} />;
            points.push(fromPoint);
        }
        var drawToPoint =
            this.props.pointMode == "to" || this.props.pointMode == "both";
        if (drawToPoint) {
            var toPointStyle = {
                ...pointStyle,
            ...
            var toPoint = <View key="to" style={toPointStyle} />;
            points.push(toPoint);
        }
        return (
...
var LineChart = React.createClass({
    ...
    _renderLineChart: function(dataset, index) {
        ...
        return (
            <View key={"lineChart-" + index} style={styles.chartContainer}>
                {margin}
                <View style={chartFlex}>{[areaContainer, lineContainer]}</View>
                {margin}
            </View>
        );
    },
    ...
    render: function() {
        ...
        return (
            <View key="chart" style={this.props.style}>
                {lineCharts}
            </View>
        );
    }
