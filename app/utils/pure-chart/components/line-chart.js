...
class LineChart extends React.Component {
    ...
    getTransform(rad, width) {
        let x =
            (0 - width / 2) * Math.cos(rad) - (0 - width / 2) * Math.sin(rad);
        let y =
            (0 - width / 2) * Math.sin(rad) + (0 - width / 2) * Math.cos(rad);
        ...
        return (
            ...
            >
                ...
                {seriesIndex === this.state.sortedData.length - 1 && (
                    <TouchableWithoutFeedback
                        onPress={() => {
                            let selectedIndex = lastCoordinate
                                ? index - 1
                                : index;
                            this.setState(
                                {
                                ...
                                () => {
                                    if (
                                        typeof this.props.onPointClick ===
                                        "function"
                                    ) {
                ...
                >
                    ...
                    <View style={StyleSheet.flatten([styles.selectedBox])}>
                        {this.state.sortedData.map(series => {
                            let dataObject =
                                series.data[this.state.selectedIndex];
                            return (
